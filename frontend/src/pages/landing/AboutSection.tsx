import { CheckCircle, Flame, Lock, Bug } from 'lucide-react';
...
const quests = [
  ...
  },
  {
    ...
    status: 'Locked',
    statusClass: 'bg-slate-950 text-text-muted',
  },
  {
    icon: Lock,
    iconClass: 'text-text-muted',
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <FadeIn>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center mt-16">
            ...
            {/* Visual */}
            <div className="bg-surface-base border border-border rounded-[20px] p-8 relative overflow-hidden">
              ...
              <div className="flex flex-col gap-2.5">
                {quests.map((q) => {
                  ...
                  return (
                    <div
                      key={q.title}
                      className={`flex items-center gap-3.5 bg-slate-800 border rounded-[10px] px-4 py-3 transition-colors ${q.active ? 'border-primary' : 'border-border'}`}
                    >
                      <Icon className={`w-5 h-5 ${q.iconClass}`} aria-hidden="true" />
                      <div>
